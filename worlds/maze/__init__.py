...
MazeMission = namedtuple("MazeMission", ["goal", "hint"])
...
class MazeWorld(object):
    def __init__(self, config):
        self.config = config
        self.cookbook = cookbook.Cookbook("worlds/maze/recipes.yaml")
        self.hints = {}
        with open("worlds/maze/hints.yaml") as hint_f:
    ...
    def sample_task(self):
        ...
        mission = MazeMission(goal, hint)
        init_state = self.sample_state_with_goal(mission.goal)
        return MazeTask(mission, init_state)
    ...
    def sample_state_with_goal(self, goal):
        ...
        scen = MazeScenario(walls, doors, keys, door_features, key_features, 
...
class MazeScenario(object):
    ...
    def init(self):
        ...
        s = MazeState(self.walls, self.doors, self.keys, (ix, iy), self)
        return s
...
class MazeState(object):
    ...
    def step(self, action):
        ...
        if (nx, ny) in self.doors and (nx, ny) in self.keys.values():
            nx, ny = x, y
        return 0, MazeState(self.walls, self.doors, n_keys, (nx, ny), self.scenario)
